...
def _get_kline_with_backoff(client, **kwargs):
    """Wrapper around client.get_kline with exponential backoff on errors/rate limits."""
    attempt = 0
    backoff = POLL_INTERVAL_SECONDS
    while True:
        try:
            return client.get_kline(**kwargs)
        except Exception as e:
            # Pode ser erro de rate limit ou rede; aplicar backoff exponencial
            attempt += 1
            wait = min(backoff * (2 ** (attempt-1)), MAX_BACKOFF_SECONDS)
            print(f"⚠️ get_kline failed (attempt {attempt}): {e}. Backing off {wait}s")
            time.sleep(wait)
...
def carregar_dados_historicos(cripto, tempo_grafico, emas, start, end, pular_velas=999, remove_velas=True):
    ...
    if velas_sem_estrutura is None:
        ...
        while start_timestamp < end_timestamp:
            resposta = _get_kline_with_backoff(cliente, symbol=cripto, interval=tempo_grafico, limit=1000, start=start_timestamp)
            velas_sem_estrutura += resposta['result']['list'][::-1]
            start_timestamp = int(velas_sem_estrutura[-1][0]) + 1000
            # Evitar bater na API muito rápido
...
def busca_velas(cripto, tempo_grafico, emas):
    ...
    resposta = _get_kline_with_backoff(cliente, symbol=cripto, interval=tempo_grafico, limit=1000)
    velas_sem_estrutura = resposta['result']['list'][::-1]
